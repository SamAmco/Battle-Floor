...
namespace Battle_Floor
{
    class Mage : EnemyBase
    {
        ...
        List<MageBlast> BlastList = new List<MageBlast>();
        List<MageBall> BallList = new List<MageBall>();
        MageShield mageShield;
        #endregion

        public Mage(Texture2D StandMove, Texture2D CastBall, Texture2D MageBallTex,
        ...
        private void HandleFrames(GameTime gameTime)
        {
            ...
            if (FrameTimer >= 30)
            {
                ...
                switch (CurrentState)
                {
                    ...
                    case "CastBall":
                        {
                            ...
                            {
                                Game1.enemies.PlaySound("MageBallCharge");
                            }
                            if (CurrentFrame.X < 7)
                                CurrentFrame.X += 1;
                    ...
                    case "Shield":
                    {
                        if (FrameIncrement)
                        {
                            ...
                            else if (CurrentFrame.X == 3 && CurrentFrame.Y == 1)
                            {
                                ...
                                mageShield = new MageShield(Shield, Pos, new Point(45, 60), new Point(0, 0),
                                    this);
                                Game1.enemies.PlaySound("MageCastShield");
                            }
                        }
                    ...
                    {
                        if (CurrentFrame.X == 0 && CurrentFrame.Y == 0)
                            Game1.enemies.PlaySound("MageBlastCharge");
                        ...
                        if (CurrentFrame.X == 0 && CurrentFrame.Y == 3)
                        {
                            if (Direction == 'L')
                            {
                                BlastList.Add(new MageBlast(MageBlastTex, new Vector2(Pos.X - 780, Pos.Y),
                                    new Point(800, 50),
                                    new Point(0, 0), Direction));
                            ...
                            {
                                BlastList.Add(new MageBlast(MageBlastTex, new Vector2(Pos.X + 50, Pos.Y),
                                    new Point(800, 50),
                                    new Point(0, 0), Direction));
                                Game1.enemies.PlaySound("MageBlast");
                            }
        ...
        {
            foreach (MageBlast m in BlastList)
            {
                m.Draw(spriteBatch);
            }
        }
        ...
        {
            if (BallList.Count() > 0)
                return BallList.Last<MageBall>().GetARect();
            else
        ...
        public Rectangle GetBRect()
        {
            if (BlastList.Count() > 0)
                return BlastList.Last<MageBlast>().GetARect();
            else
                return new Rectangle(0, 0, 0, 0);
        }
        ...
        public void BallHit(Vector2 CharPos)
        {
            BallList.Last<MageBall>().Hit(CharPos);
        }
        public void BlastHit()
        {
            BlastList.Last<MageBlast>().Hit();
        }
        public bool ColDet()
        {
            if (BallList.Last<MageBall>().CurrentFrame.X == 0)
                return true;
            else
                return false;
        }
        ...
        public void CombatHit(int Damage, bool Lightning)
        {
            if (Lightning)
                Health -= Damage;
            else if (mageShield != null)
        ...
        public bool Dispose()
        {
            if (Destroy)
            {
                Game1.mainCharacter.CreateDust(30,
        ...
        {
            if (BlastList.Last<MageBlast>().IsInclined())
                return true;
            else
                return false;
        }
